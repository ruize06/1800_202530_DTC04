...
import { createEditTaskForm, cancelEditTaskForm, editTaskFromForm, deleteTaskFromForm } from '/src/components/Tasks/edit-task.js';
        ...
        onSnapshot(doc(db, "tasks", task.id), (docSnap) => {
            if (docSnap.exists()) {
                ...
                if (taskJSON["completed"] == true || taskJSON["completed"] == "true") {
                    ...
                    task_list.prepend(task_box);
                    task_box.classList.remove("line-through", "opacity-60");
                }
